...
import { useGetPost, useSearchPost } from '@/lib/react-query/queryAndMutation'
    ...
    return (
        <div className='explore-container'>
            ...
            <div className='flex flex-wrap gap-9 w-full max-w-5xl'>
                {shouldShowSearchResult ? (<SearchResults isSearchFetching={isSearchFetching} searchedPosts={searchedPost} />) :
                    shouldShowPost ? (<p className='text-light-4 mt-10 text-center w-full'>end of posts</p>) : posts?.pages.map((item, index) => (<GridPostList key={`page-${index}`} posts={item.documents} />))
                }

            </div>
            {hasNextPage && !searchValue && (<div ref={ref} className='mt-10'>
                <Loader />
            </div>)}
        </div>
    )
